...
import {
  ...
  SortOrder,
} from './dto/quotation-list-query.dto';
...
function getDefaultCostRatio(itemType = '', description = '') {
  ...
  if (
    desc.includes('venue') ||
    desc.includes('ballroom') ||
    desc.includes('plaza')
  )
    return DEFAULT_COST_RATIOS.VENUE;
  if (
    desc.includes('floral') ||
    desc.includes('decor') ||
    desc.includes('flower')
  )
    return DEFAULT_COST_RATIOS.FLORAL;
  if (
    ...
    return DEFAULT_COST_RATIOS.CATERING;
  if (
    ...
    return DEFAULT_COST_RATIOS.ENTERTAINMENT;
...
export interface QuotationAccessScope {
  createdBy?: number;
  wonOnly?: boolean;
}
...
export class QuotationService {
  ...
  async create(createDto: CreateQuotationDto, userId = 1) {
    this.logger.log('QTN DB MODE: Creating quotation in PostgreSQL');
    ...
    if (createDto.parent_quotation_id) {
      const parentQuote = await this.prisma.quotation.findFirst({
        where: {
          quotation_id: BigInt(createDto.parent_quotation_id),
          is_active: true,
        },
      });
      if (!parentQuote) {
        throw new NotFoundException(
          `Parent quotation with ID ${createDto.parent_quotation_id} not found`,
        );
      }
    }
    ...
    if (createDto.lines && createDto.lines.length > 0) {
      for (const line of createDto.lines) {
        if (line.tax_rule_id) {
          ...
          if (!taxRule) {
            throw new NotFoundException(
              `Tax Rule with ID ${line.tax_rule_id} not found`,
            );
          }
        }
        if (line.item_type === 'PACKAGE') {
          ...
          if (!pkg) {
            throw new NotFoundException(
              `Package with ID ${line.ref_id} not found`,
            );
          }
          ...
          if (!rateCard) {
            throw new NotFoundException(
              `Rate Card with ID ${line.ref_id} not found`,
            );
          }
        }
      }
    ...
    const quotation = await this.prisma.quotation.create({
      data: {
        ...
        expires_at: createDto.expires_at
          ? new Date(createDto.expires_at)
          : null,
        parent_quotation_id: createDto.parent_quotation_id
          ? BigInt(createDto.parent_quotation_id)
          : null,
        is_active: true,
  ...
  async findAll(
    ...
    const accessFilter = scope.createdBy
      ? { created_by: BigInt(scope.createdBy) }
      : scope.wonOnly
    ...
    const normalizedSortOrder = String(
      query.sortOrder || SortOrder.DESC,
    ).toLowerCase();
    const order = normalizedSortOrder === 'asc' ? 'asc' : 'desc';
  ...
  async findOne(id: number, scope: QuotationAccessScope = {}) {
    const quotation = await this.prisma.quotation.findFirst({
      where: {
        quotation_id: BigInt(id),
        is_active: true,
        ...(scope.createdBy ? { created_by: BigInt(scope.createdBy) } : {}),
        ...(scope.wonOnly ? { status: QuotationStatus.ACCEPTED } : {}),
      },
      include: {
        lead: {
          select: { lead_id: true, name: true },
  ...
  async update(id: number, updateDto: UpdateQuotationDto) {
    ...
    if (quotation.status === QuotationStatus.PENDING_APPROVAL) {
      const keys = Object.keys(updateDto).filter(
    ...
    if (updateDto.expires_at)
      dataToUpdate.expires_at = new Date(updateDto.expires_at);

    const hasMetadataChanges =
      updateDto.currency !== undefined || updateDto.expires_at !== undefined;
    if (
      hasMetadataChanges &&
      !updateDto.status &&
  ...
  async addLine(quotationId: number, lineDto: CreateQuotationLineDto) {
    ...
    if (lineDto.tax_rule_id) {
      ...
      if (!taxRule) {
        throw new NotFoundException(
          `Tax Rule with ID ${lineDto.tax_rule_id} not found`,
        );
      }
    }
    ...
    if (lineDto.item_type === 'PACKAGE') {
      ...
      if (!pkg) {
        throw new NotFoundException(
          `Package with ID ${lineDto.ref_id} not found`,
        );
      }
      ...
      if (!rateCard) {
        throw new NotFoundException(
          `Rate Card with ID ${lineDto.ref_id} not found`,
        );
      }
    }
  ...
  async updateLine(
    quotationId: number,
    lineId: number,
    updateLineDto: UpdateQuotationLineDto,
  ) {
    const quotation = await this.findOne(quotationId);
    if (quotation.status === QuotationStatus.PENDING_APPROVAL) {
      throw new BadRequestException('Quotation is locked pending approval');
    ...
    if (updateLineDto.tax_rule_id !== undefined) {
      if (updateLineDto.tax_rule_id !== null) {
        const taxRule = await this.prisma.taxRule.findFirst({
          where: {
            tax_rule_id: BigInt(updateLineDto.tax_rule_id),
            is_active: true,
          },
        });
        if (!taxRule) {
          throw new NotFoundException(
            `Tax Rule with ID ${updateLineDto.tax_rule_id} not found`,
          );
        }
      }
    }
    ...
    const currentItemType =
      updateLineDto.item_type !== undefined
        ? updateLineDto.item_type
        : line.item_type;
    const currentRefId =
      updateLineDto.ref_id !== undefined ? updateLineDto.ref_id : line.ref_id;

    if (
      updateLineDto.item_type !== undefined ||
      updateLineDto.ref_id !== undefined
    ) {
      if (currentItemType === 'PACKAGE') {
        ...
        if (!pkg) {
          throw new NotFoundException(
            `Package with ID ${currentRefId} not found`,
          );
        }
        ...
        if (!rateCard) {
          throw new NotFoundException(
            `Rate Card with ID ${currentRefId} not found`,
          );
        }
      }
    }
    ...
    if (updateLineDto.rate !== undefined)
      dataToUpdate.rate = updateLineDto.rate;
    if (updateLineDto.description !== undefined)
      dataToUpdate.description = updateLineDto.description;
    if (updateLineDto.tax_rule_id !== undefined)
      dataToUpdate.tax_rule_id = updateLineDto.tax_rule_id
        ? BigInt(updateLineDto.tax_rule_id)
        : null;
    if (updateLineDto.item_type !== undefined)
      dataToUpdate.item_type = updateLineDto.item_type;
    if (updateLineDto.ref_id !== undefined)
      dataToUpdate.ref_id = BigInt(updateLineDto.ref_id);

    const currentQty =
      updateLineDto.qty !== undefined ? updateLineDto.qty : Number(line.qty);
    const currentRate =
      updateLineDto.rate !== undefined
        ? Number(updateLineDto.rate)
        : Number(line.rate);
    const currentCost =
      updateLineDto.cost !== undefined
        ? Number(updateLineDto.cost)
        : Number(line.cost);
    const currentDescription =
      updateLineDto.description !== undefined
        ? updateLineDto.description
        : line.description;
    dataToUpdate.cost = resolveUnitCost(
      currentRate,
      currentCost,
      currentItemType,
      currentDescription,
  ...
  async calculate(quotationId: number) {
    ...
    const pricingLines: PricingLineInput[] = quote.lines.map((line) => ({
      ...
      taxPercent: line.tax_rule
        ? Number(line.tax_rule.rate_percent)
        : DEFAULT_TAX_PERCENT,
