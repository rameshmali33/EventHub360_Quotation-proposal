...
function mapQuotationToDto(quotation: any): any {
  ...
  return {
    ...
    lines: (serialized.lines || []).filter((line: any) => line.is_active !== false).map((line: any) => ({
      lineId: line.line_id,
...
export class ProposalService {
  ...
  async create(quotationId: number) {
    ...
    if (quotation.status !== QuotationStatus.APPROVED) {
      throw new BadRequestException(`Proposal can be generated only after quotation approval. Current status: ${quotation.status}`);
    }
  ...
  async findOneByQuotationId(quotationId: number) {
    ...
    if (!proposal) {
      throw new NotFoundException(`Proposal for quotation ${quotationId} not found`);
    }
    return mapDbProposalToDto(proposal);
  }

  async send(quotationId: number, sendDto: SendProposalDto) {
    const proposal = await this.findOneByQuotationId(quotationId);
    if (proposal.status !== 'DRAFT') {
      throw new BadRequestException(`Proposal cannot be sent. Current status: ${proposal.status}`);
    }
    ...
    await this.quotationService.update(quotationId, { status: QuotationStatus.SENT });
  ...
  async findByHash(publicHash: string) {
    ...
    const quotation = await this.quotationService.findOne(Number(proposal.quotation_id));
    const signature = proposal.signature ? mapDbSignatureToDto(proposal.signature) : null;
  ...
  async sign(publicHash: string, signatureDto: ProposalSignatureDto, ipAddress: string) {
    const proposal = await this.prisma.proposal.findFirst({
      where: { public_hash: publicHash }
    });
    if (!proposal) {
    ...
    if (proposal.status === 'ACCEPTED' || proposal.status === 'REJECTED') {
      throw new BadRequestException(`Proposal is read-only (Current Status: ${proposal.status})`);
    }
  ...
  async accept(publicHash: string, ipAddress: string) {
    const proposal = await this.prisma.proposal.findFirst({
      where: { public_hash: publicHash },
      include: {
        views: true,
    ...
    if (!isViewed && proposal.status !== 'VIEWED') {
      throw new BadRequestException('Proposal must be viewed before it can be accepted');
    }

    if (!proposal.signature) {
      throw new BadRequestException('Proposal must be signed before it can be accepted');
    }

    await this.quotationService.update(Number(proposal.quotation_id), { status: QuotationStatus.ACCEPTED });
  ...
  async reject(publicHash: string, actionDto: ProposalActionDto, ipAddress: string) {
    const proposal = await this.prisma.proposal.findFirst({
      where: { public_hash: publicHash }
    });
    if (!proposal) {
    ...
    await this.quotationService.update(Number(proposal.quotation_id), { status: QuotationStatus.REJECTED });
